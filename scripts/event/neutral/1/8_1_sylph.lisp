...
(dialog (tr "You encounter a Sylph civilian vessel. The occupant is fleeing..."))
    ...
    (if temp
        (progn
          ...
          (if (or (equal (choice 2) 1) (< (coins) 600))
              (join (tr "The deserter joined your crew!"))
              (progn
                (coins-set (- (coins) 600))
                (join (tr "The deserter joined your crew. Hungry, he ate 600@ of your food supplies!")))))
        (progn
          (dialog (tr "Sadly, there's no room..."))
          (defn on-dialog-closed ()
            (dialog (tr "<c:Sylph Artisan:46>No room in your castle? Hold on, I've got some supplies, I'll help out..."))
            (defn on-dialog-closed ()
              (alloc-space 'ladder)
              (sel-input 'ladder
                         (tr "Place ladder (1x2):")
                         (lambda (isle x y)
                           ...
                           (dialog (tr "<c:Sylph Artisan:46>Thanks! I'll try to help out however I can!"))
                           (defn on-dialog-closed ()
                             (join (tr "The deserter joined your crew!"))
                             (defn on-dialog-closed ()
