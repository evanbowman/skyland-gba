...
(dialog (tr "A heavily armed marauder fortress approaches. Its captain demands to speak with you."))
...
(defn on-dialog-accepted ()
  (if (< (coins) 600)
      (progn
        ...
        (attack-player (tr "<c:redbeard:12>That's not enough, load the cannons!!!")))
      (progn
        (adventure-log-add 13 (list 600))
        (coins-add -600)
        (dialog (tr "<c:redbeard:12>Heh. I think you made the smart decision."))
        (exit))))


(defn on-dialog-declined ()
  (adventure-log-add 14 '())
  (attack-player (tr "<c:redbeard:12>Whaatt!! Load the cannons!!!")))
