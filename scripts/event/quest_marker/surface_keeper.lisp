...
(dialog (tr "You reach the destination that the surface keeper marked on your map! <B:0> Beyond the clouds, pulses of strange energy light up the toxic atmosphere..."))
    ...
    (if id
        (progn
          (let ((tab (eval-file "/scripts/config/room_tab.lisp"))
            ...
            (dialog (tr "<c:Surface Keeper:43>The readingsss grow stronger... <B:0> Beneath usss lies a cache of ancient weaponsss, their containment failing. <B:0> I must descend to stabilize them, before their radiation spreadsss upward. <B:0> Take thisss payment for your aid. (2000@ and one random block) "))
              ...
              (let ((sym0 (get sel 0)))
                ...
                (sel-input sym0
                           ...
                           (lambda (isle x y)
                             ...
                             (dialog (tr "<c:Surface Keeper:43>The ancient machinesss require my attention..."))
                             (map (lambda (chr)
                                    (if (equal id (lookup 'id (cddr chr)))
                                        (chr-del (player) (car chr) (cadr chr))))
                                  (chrs (player)))
                             (defn on-dialog-closed ()
                               (dialog (tr "The surface keeper departed!"))
                               (setq on-dialog-closed exit))))))))
      (progn
        (dialog (tr "Unfortunately, it seems the surface keeper is no longer aboard your island..."))
        (exit)))))
