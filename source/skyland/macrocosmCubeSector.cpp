...
void terrain::CubeSector::rotate()
{
...
void terrain::CubeSector::shadowcast()
{
    for (int z = 0; z < z_limit; ++z) {
        for (int x = 0; x < 8; ++x) {
            for (int y = 0; y < 8; ++y) {
...
const terrain::Block&
terrain::CubeSector::get_block(const Vec3<u8>& coord) const
{
...
terrain::Block& terrain::CubeSector::ref_block(const Vec3<u8>& coord)
{
    return blocks_[coord.z][coord.x][coord.y];
}
...
void terrain::CubeSector::set_repaint(bool value)
{
    for (auto& slab : blocks_) {
        for (auto& slice : slab) {
            for (auto& block : slice) {
...
void terrain::CubeSector::render_setup(Platform& pfrm)
{
