...
ViewBudgetScene::ViewBudgetScene() : s_(allocate_dynamic<State>("budget-info"))
{
}
...
void ViewBudgetScene::show(Platform& pfrm, macro::EngineImpl& state)
{
    if (not s_->heading_) {
        s_->heading_.emplace(pfrm, OverlayCoord{1, 1});
        s_->heading_->assign(SYSTR(macro_fiscal_budget)->c_str());
    ...
    auto append_value = [&](Float fval) {
        ...
        auto add_space = (st.x - 2) - (s_->lines_.back().len() + tl);
        if (add_space > 0) {
            while (add_space) {
    ...
    if (not s_->lines_.full()) {
        ...
        s_->lines_.back().append(SYSTR(macro_fiscal_total)->c_str());
...
ScenePtr<Scene>
ViewBudgetScene::update(Platform& pfrm, App& app, Microseconds delta)
{
    if (exit_) {
