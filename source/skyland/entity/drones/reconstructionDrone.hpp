...
public:
    ...
    void enter(Scene& prev) override
    {
        ...
        Text::print(SYSTR(automate_reconstruction_prompt_1)->c_str(), OverlayCoord{});
        show_count();
    }
...
private:
    Buffer<ReconstructionQueueEntry, reconstruction_queue_size> reconstruction_queue_;
};
...
{
public:
    ReconstructionDrone(Island* parent, Island* destination, const RoomCoord& grid_pos)
                    ...
                    if (recons_index_ < db->rq_.size()) {
                        ...
                        if (not parent()->get_room(pos)) {
                            ...
                            if (cost <= APP.coins()) {
                                ...
                                if (not PLATFORM.speaker().is_sound_playing("build0")) {
            ...
            if (timer_ > milliseconds(400)) {
                ...
                if (auto db = room->cast<DroneBay>()) {
                    for (int i = 0; i < db->rq_.size(); ++i) {
                        auto entry = db->rq_[i];
                        RoomCoord pos = {entry.x_, entry.y_};
                        bool destroyed = minimap::player_destroyed_rooms.get(pos.x, pos.y);
